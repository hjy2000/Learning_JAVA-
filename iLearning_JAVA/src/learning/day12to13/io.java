...
class File1
{
    public void test()
    {
        File f=new File("G:\\Java projects\\iLearning_JAVA\\src\\learning\\day12to13\\t.txt");//这里用/可以（unix写法） 或者\\转义也可以（win写法）
...
//        f.renameTo(new File("G:\\Java projects\\iLearning_JAVA\\src\\learning\\day12to13\\ttt.txt"));//重命名
...
//        f.delete();//删除文件
    ...
    public void test5()
    {
        //文件字节输入流
        try {
            FileInputStream f=new FileInputStream("G:\\Java projects\\iLearning_JAVA\\src\\learning\\day12to13\\t.txt");
            byte[] b=new byte[100];
            int len=0;
            while((len=f.read(b))!=-1) {
//                f.read(b);//read方法返回值是读取文件的长度 到达文件尾后再向后读一位就是返回-1
    ...
    public void test6()
    {
        ...
        try {
            // 追加模式
            FileOutputStream f=new FileOutputStream("G:\\Java projects\\iLearning_JAVA\\src\\learning\\day12to13\\t123.txt",true);
    ...
    public void copy()
    {
        //字节流复制文件
        try {
            FileInputStream fin=new FileInputStream("G:\\Java projects\\iLearning_JAVA\\src\\learning\\day12to13\\t123.txt");
            FileOutputStream fout=new FileOutputStream("G:\\Java projects\\iLearning_JAVA\\src\\learning\\day12to13\\t123_copy.txt");
            byte [] b=new byte[50];
            int len=0;
    ...
    public void bOutputStream()
    {

    }

    public void bCopyStream()
    {
    ...
    public void bCharCopy()
    {

    }
}
//注意 文件字节流其实非常通用 大部分文件都可以转换为二进制再操作 那就支持
public class io {
