...
// day7_8后面再回来看 没完全听明白
// static修饰的方法不能被重写，只能被继承。而属性也是只有重新声明和继承,没有重写的概念。
...
public class test1 {
  public static void main(String[] args) {
    ...
    // System.out.println(s.age);//子类不能使用父类中private的属性和方法 和C++差不多 子类父类在同一个包中的话 只要不是private都能使用
    // 如果不在同一个包中 子类只能使用protected和public
    ...
    // 下面是对象多态的例子
    person e =
        new Student(); // 父类引用对象可以指向子类的实例 子类可以看作特殊父类 子类对象向上转型 此时e不能访问子类的属性和方法 只是子类对象实例的一部分 这叫动态绑定
...
class Uni_student extends Student {
  ...
  public void test() {
    ...
    super.setAge(1);
    // 方法被重写时，super执行直接父类的方法 若直接父类有对间接父类的方法重写 重写时也是使用的super方法则可以访问到间接子类中的方法 哪一层没有super的话
    // 最底层子类就只能super到断的那一层
